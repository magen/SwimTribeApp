...
export async function loadAnchors(): Promise<Anchors> {
  try {
    ...
    console.warn('[Anchors] Failed to load anchors', err);
    return {};
  }
}
...
export async function saveAnchors(next: Anchors) {
  try {
    ...
    console.warn('[Anchors] Failed to save anchors', err);
  }
}
...
export async function clearAnchors() {
  try {
    ...
    console.warn('[Anchors] Failed to clear anchors', err);
  }
}
